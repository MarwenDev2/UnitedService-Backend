...
class DemandeCongeController {
    ...
    public ResponseEntity<DemandeCongeDTO> updateRHDecision(@PathVariable Long id, @RequestParam boolean approved) {
        try {
            ...
            // Also send a test push notification for debugging
    ...
    @GetMapping("/check-eligibility/{workerId}")
    public ResponseEntity<String> checkEligibility(
            @PathVariable Long workerId,
            @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate startDate,
            @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate endDate) {
        String result = congeService.checkEligibility(workerId, startDate, endDate);
        if ("OK".equals(result)) {
