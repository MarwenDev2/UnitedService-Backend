...
public class MissionRequestController {
    ...
    public ResponseEntity<MissionRequestDTO> create(
            @RequestParam List<Long> workerIds,
            @RequestParam String destination,
            @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate missionDate,
            @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate endDate
    ) {
        try {
            MissionRequestDTO savedMission = missionService.submitMissionRequest(workerIds, destination, missionDate, endDate);
            return ResponseEntity.ok(savedMission);
        } catch (RuntimeException e) {
            logger.error("Error creating mission request for workers: {}", workerIds, e);
            return ResponseEntity.badRequest().body(null);
        }
    }

    @GetMapping("/{id}/with-workers")
    public ResponseEntity<MissionRequestDTO> getByIdWithWorkers(@PathVariable Long id) {
        return missionService.getMissionRequestWithWorkers(id)
