...
class WorkerController {
    ...
    @PostMapping("/{id}/related-files")
    public ResponseEntity<WorkerDTO> uploadRelatedFiles(@PathVariable Long id, @RequestParam("files") MultipartFile[] files) {
        Worker updatedWorker = workerService.uploadRelatedFiles(id, files);
        return ResponseEntity.ok(modelMapper.map(updatedWorker, WorkerDTO.class));
    }

    @GetMapping("/{id}/related-files")
    public ResponseEntity<String> getRelatedFiles(@PathVariable Long id) {
        return workerService.getWorkerById(id)
                .map(worker -> ResponseEntity.ok(worker.getRelatedFilesPath() != null ? worker.getRelatedFilesPath() : ""))
                .orElse(ResponseEntity.notFound().build());
    }
}